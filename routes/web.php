...
Route::delete('events/{event}/favorites', 'FavoriteController@deletes');

#Toggle favorite reply
Route::post('replies/{reply}/favorites', 'FavoriteReplyController@store');
Route::delete('replies/{reply}/favorites', 'FavoriteReplyController@deletes');


#replies
Route::post('/events/{event}/replies','ReplyController@store')->name('reply.store');
Route::delete('/replies/{reply}','ReplyController@deletes')->name('reply.destroy');
Route::patch('/replies/{reply}','ReplyController@change')->name('reply.update');

#Photo
Route::post('events/{event}/photos', 'PhotoController@store')->name('photos.store');
Route::delete('photos/{photo}', 'PhotoController@destroy')->name('photos.deletes');
